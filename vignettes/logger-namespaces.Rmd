...
Every `logger::log_*()` call in `stocktipr` passes an explicit `namespace` string. The full set, registered in `app_set_log_threshold()`, is:
...
The `"stocktipr/<module>"` convention means you can silence one noisy module while keeping the others verbose — see the examples below.
...
- `?stocktipr::app_set_log_threshold`
- `?stocktipr::with_logging`